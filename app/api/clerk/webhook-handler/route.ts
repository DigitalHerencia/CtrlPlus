...
function fromMillis(timestamp?: number): Date | undefined {
...
async function handleEmailEvent(eventType: string, data: unknown, eventId: string): Promise<void> {
  ...
  const toEmail = getFirstStringPath(data, [["to_email_address"], ["recipient_email_address"], ["email_address"], ["to"]])
...
async function handleSubscriptionEvent(eventType: string, data: unknown): Promise<void> {
  ...
  const tenantId = getFirstStringPath(data, [["public_metadata", "tenantId"], ["metadata", "tenantId"], ["tenant_id"]])
