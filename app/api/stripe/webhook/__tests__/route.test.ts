...
describe('POST /api/stripe/webhook', () => {
    ...
    it('returns 200 for ignored event types', async () => {
        ...
        const response = await POST(buildRequest('{"id":"evt_1","type":"payment_intent.created"}', 'sig_123') as never)
    ...
    it('returns 200 when payment is processed', async () => {
        ...
        const response = await POST(buildRequest('{"id":"evt_1","type":"checkout.session.completed"}', 'sig_123') as never)
    ...
    it('returns 500 when the processor throws', async () => {
        mocks.constructWebhookEvent.mockReturnValue({ id: 'evt_1', type: 'checkout.session.completed' })
