    ...
    <div className="max-w-6xl space-y-8">
        ...
        <CardContent>
          <div className="flex flex-wrap gap-6">
            <div>
              ...
              <Badge variant={stripeStatus.failureCount > 0 ? "destructive" : "default"}>
            ...
            <div>
              ...
              <span>
                {stripeStatus.lastFailure?.processedAt
                  ? new Date(stripeStatus.lastFailure.processedAt).toLocaleString()
                  : "—"}
              </span>
            </div>
            <div>
              ...
              <span>
                {stripeStatus.lastSuccess?.processedAt
                  ? new Date(stripeStatus.lastSuccess.processedAt).toLocaleString()
                  : "—"}
              </span>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Failed events are eligible for manual retry or dismissal. Use recovery tools below.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <PlatformRecoveryActions
        clerkFailureIds={webhookOverview.clerk.recentFailures.map((f: WebhookFailureDTO) => f.id)}
        stripeFailureIds={webhookOverview.stripe.recentFailures.map((f: WebhookFailureDTO) => f.id)}
