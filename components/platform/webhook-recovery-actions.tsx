...
function ResetWebhookFailuresForm({
    ...
    const handleSubmit = form.handleSubmit((values) => {
        ...
        if (!parsed.success) {
            form.setError('root', {
                message:
                    parsed.error.issues[0]?.message ?? 'Unable to validate the recovery request.',
            })
            return
        }
            ...
            <CardContent className="space-y-4">
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <FieldDescription className="text-neutral-400">
                        Only failed or stale processing events that still belong to the current
                        source will be touched.
                    </FieldDescription>
                    <FieldError
                        errors={
                ...
                <CardHeader className="gap-3">
                    <div className="flex items-start justify-between gap-3">
                        <div className="space-y-1">
                            ...
                            <CardDescription className="text-neutral-300">
                                Mark processing rows older than {staleThresholdMinutes} minutes as
                                failed so controlled replay can resume.
                            </CardDescription>
                        </div>
                        <div className="inline-flex min-w-12 items-center justify-center border border-neutral-700 px-3 py-1 text-sm font-medium text-neutral-100">
                            {staleTotal}
                ...
                <CardContent className="space-y-4">
                    ...
                    <form onSubmit={handleClearStaleProcessing} className="space-y-4">
                        <FieldDescription className="flex items-start gap-2 text-neutral-400">
                            ...
                            This action keeps the existing platform-admin authorization and route
                            revalidation rules intact.
                        </FieldDescription>
                        <FieldError
                            errors={
