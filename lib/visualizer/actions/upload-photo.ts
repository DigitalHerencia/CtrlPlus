...
import { assertApprovedRemoteHost } from '../preview-pipeline'
...
export async function uploadVehiclePhoto(input: UploadPhotoInput): Promise<VisualizerPreviewDTO> {
    ...
    const sourceAssetVersion =
        textureAsset?.version ?? deriveAssetVersion(textureAsset?.url ?? null)
    ...
    // Always upload to Cloudinary if not already a Cloudinary URL
    if (customerPhotoUrl.startsWith('data:')) {
    ...
    } else if (!customerPhotoUrl.includes('cloudinary.com')) {
        // If remote image, fetch and re-upload to Cloudinary
        const url = new URL(customerPhotoUrl)
