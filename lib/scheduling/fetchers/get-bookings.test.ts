...
import { getBookingById, getBookings, getUpcomingBookingCount } from './get-bookings'
...
describe('scheduling booking fetchers', () => {
    ...
    it('returns only the current customer bookings when the user cannot read all bookings', async () => {
        ...
        mocks.hasCapability.mockImplementation((_, capability) => capability === 'scheduling.read.own')
    ...
    it('counts upcoming bookings in the active scope', async () => {
        ...
        await expect(getUpcomingBookingCount(new Date('2026-03-01T00:00:00.000Z'))).resolves.toBe(
            1
        )
