...
- Establish a durable `.codex` scaffold that agents can use repeatedly during the refactor.
...
- Primary: future Codex agents executing bounded refactor passes
...
- Future agents can identify the next refactor task from `.codex/execution/refactor-backlog.json`.
...
- Refactor planning and tracking artifacts under `.codex/`
...
- Repo-local instructions must mention `.codex/contracts/` and `.codex/execution/` as authoritative layers.

## Acceptance Criteria

- Given a new agent session, when the agent opens `.codex/README.md`, then it can identify the source docs, contracts, and execution files for the refactor program.
