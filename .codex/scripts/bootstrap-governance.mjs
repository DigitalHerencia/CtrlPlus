...
const labelsConfig = readJson('.codex/manifests/github/labels.json');
const manifestPath = resolveManifestPath();
const manifest = readJson(manifestPath);
const projectConfig = readJson('.codex/manifests/github/project-v2.json');
...
if (unknownTaskLabels.size > 0) {
  throw new Error(
    `${manifestPath} references labels missing from .codex/manifests/github/labels.json: ${Array.from(
      unknownTaskLabels
    ).join(', ')}`
  );
}
...
function resolveManifestPath() {
  const candidates = ['.codex/manifests/github/task-manifest.json', 'task-manifest.json'];
