...
describe('PlatformRecoveryActionsClient', () => {
    ...
    it('runs stale-lock cleanup and shows the success summary', async () => {
        ...
        await expect(
            screen.findByText(/Released 3 stale processing locks across Clerk \(1\) and Stripe \(2\)\./)
    ...
    it('replays Stripe failures and forwards the selected ids', async () => {
        ...
        await expect(
            screen.findByText(/Stripe replay finished: 1 replayed, 0 ignored, 1 non-replayable, 0 failed\./)
        ).resolves.toBeVisible()
    })
