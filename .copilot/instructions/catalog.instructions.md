...
- Update `.copilot/execution/backlog.json`, `.copilot/execution/progress.json`, and `.copilot/execution/validation.json` when catalog scope or validation state changes materially.
- Record any change to asset roles, DTO shape, or handoff semantics in `.copilot/contracts/catalog-visualizer.contract.yaml`.
